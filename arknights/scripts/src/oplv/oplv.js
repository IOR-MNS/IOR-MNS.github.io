...
const calc = function (formdata) {
	...
	{
		consumption.lmd += db.oplv.elitePromotion.lmdTable['elite_' + String(i)][formdata.rarity - 1]
	}
	...
	temp = db.oplv.stage.exp[formdata.stage.exp]
	...
	temp = db.oplv.stage.lmd[formdata.stage.lmd]
	...
	temp = db.oplv.stage.exp[formdata.stage.exp].sanity
	...
	temp = db.oplv.stage.lmd[formdata.stage.lmd].sanity
...
const getFormData = function () {
	...
	formdata.resource = {}
	formdata.resource.exp = Number($('#resource_expCard_tier1').val()) * db.oplv.expCard.tier_1
	formdata.resource.exp += Number($('#resource_expCard_tier2').val()) * db.oplv.expCard.tier_2
	formdata.resource.exp += Number($('#resource_expCard_tier3').val()) * db.oplv.expCard.tier_3
	formdata.resource.exp += Number($('#resource_expCard_tier4').val()) * db.oplv.expCard.tier_4
...
const copyResult = function (event) {
    gtag('event', '결과 복사', {
        'event_category': '레벨링 계산기'
    })
    
	// 개행문자 보존하며 텍스트 추출
	var resultText = $('.result-panel').text()
	...
	try {
		...
		showTooltip('성공적으로 복사하였습니다', event.target, 'success')
        
	} catch (error) {
        console.error(error)
        showTooltip('복사에 실패하였습니다', event.target, 'error')
	}
...
const showResult = function () {
	...
	// 기대치
	htmlText += '<div class="panel result-panel">'
	htmlText += '<br>'
	htmlText += '<div class="result-panel-subtitle">'
	htmlText += '기대치 ▸ '
	htmlText += '<span class="tag result-item">' + String(result.sanity.total.expectation) + ' 이성</span><br>\n'
	htmlText += '</div>'
	htmlText += '<hr class="dotted">'
	htmlText += '<span style="color: #777">경험치</span> ' + result.stage.exp.name + ' ▸ ' + '<span class="tag result-item">' + result.stage.exp.expectation + '회 클리어</span><br>\n'
	htmlText += '<span style="color: #777">용문폐</span> ' + result.stage.lmd.name + ' ▸ ' + '<span class="tag result-item">' + result.stage.lmd.expectation + '회 클리어</span><br>\n\n'
	htmlText += '</div>'
	...
	if ((result.stage.exp.best != result.stage.exp.expectation) || (result.stage.lmd.best != result.stage.lmd.expectation))
	{
		// 최소치(best)
		//htmlText += '<br>'
		htmlText += '<div class="panel result-panel" style="color: #1c70b0">'
		htmlText += '<br>'
		htmlText += '<div class="result-panel-subtitle">'
		htmlText += '최소치 ▸ '
		htmlText += '<span class="tag result-item">' + String(result.sanity.total.best) + ' 이성</span><br>\n'
		htmlText += '</div>'
		htmlText += '<hr class="dotted">'
		htmlText += '<span style="color: #777">경험치</span> ' + result.stage.exp.name + ' ▸ ' + '<span class="tag result-item">' + result.stage.exp.best + '회 클리어</span><br>\n'
		htmlText += '<span style="color: #777">용문폐</span> ' + result.stage.lmd.name + ' ▸ ' + '<span class="tag result-item">' + result.stage.lmd.best + '회 클리어</span><br>\n\n'
		htmlText += '</div>'
	}
	...
	if ((result.stage.exp.worst != result.stage.exp.expectation) || (result.stage.lmd.worst != result.stage.lmd.expectation))
	{
		// 최대치 (worst)
		//htmlText += '<br>'
		htmlText += '<div class="panel result-panel" style="color: #c70808">'
		htmlText += '<br>'
		htmlText += '<div class="result-panel-subtitle">'
		htmlText += '최대치 ▸ '
		htmlText += '<span class="tag result-item">' + String(result.sanity.total.worst) + ' 이성</span><br>\n'
		htmlText += '</div>'
		htmlText += '<hr class="dotted">'
		htmlText += '<span style="color: #777">경험치</span> ' + result.stage.exp.name + ' ▸ ' + '<span class="tag result-item">' + result.stage.exp.worst + '회 클리어</span><br>\n'
		htmlText += '<span style="color: #777">용문폐</span> ' + result.stage.lmd.name + ' ▸ ' + '<span class="tag result-item">' + result.stage.lmd.worst + '회 클리어</span><br>\n\n'
		htmlText += '</div>'
	}
	...
	htmlText += '필요 경험치 합계 ▸' + ' ' + '<span class="tag result-item">' + String(result.required.exp) + ' Exp.</span><br>\n'
	htmlText += '필요 용문폐 합계 ▸' + ' ' + '<span class="tag result-item">' + String(result.required.lmd) + ' LMD</span><br>\n'
	htmlText += '<br>\n'
	htmlText += '소모 경험치 합계 ▸' + ' ' + '<span class="tag result-item">' + String(result.consumption.exp) + ' Exp.</span><br>\n'
	htmlText += '소모 용문폐 합계 ▸' + ' ' + '<span class="tag result-item">' + String(result.consumption.lmd) + ' LMD</span><br>\n'
	htmlText += '<br>'
	htmlText += '</div>'
...
const getMaxLevel = function (elite, rarity) {
	return db.oplv.maxLevel['elite_' + elite][rarity - 1]
}

var fixCurrentLevelToMax = false
...
const btnClicked = function (e) {
	...
	// 최대 레벨로 고정 / 해제
	if (type == 'fix-current-level-to-max-btn') {
        gtag('event', '최대 레벨 고정', {
            'event_category': '레벨링 계산기'
        })
        
		fixCurrentLevelToMax = ! fixCurrentLevelToMax
		
		// 최대 레벨로 고정됐다면, 최대 레벨로 설정
		if (fixCurrentLevelToMax)
		{
			// 버튼 색상 변경
			$('[name="fix-current-level-to-max-btn"]').addClass('checked')
			// 최대 레벨로 설정
			setCurrentLevelToMax()
			$('#from_level').trigger('change')
		}
		// 최대 레벨 고정이 해제됐다면, 1레벨로 초기화.
		else
		{
			// 버튼 색상 변경
			$('[name="fix-current-level-to-max-btn"]').removeClass('checked')
			// 1레벨로 설정
			$('#from_level').val('1')
			$('#from_level').trigger('change')
		}
	...
	if (type == 'show-detail-info-panel-btn') {
        gtag('event', '상세 정보 입력란 표시', {
            'event_category': '레벨링 계산기'
        })
        
		// 상세 정보 입력란 표시
		$('#detail-info-panel').css('display', 'block')
		// '상세 정보 입력란 숨기기' 버튼 표시
		$('.hide-detail-info-panel-btn').css('display', 'inline-block')
		// '상세 정보 입력란 보이기' 버튼 숨김
		$('.show-detail-info-panel-btn').css('display', 'none')
	}
	
	// 상세 정보 입력란을 숨김
	if (type ==	'hide-detail-info-panel-btn') {
        gtag('event', '상세 정보 입력단 숨김', {
            'event_category': '레벨링 계산기'
        })
        
		// 상세 정보 입력란 숨김
		$('#detail-info-panel').css('display', 'none')
		// '상세 정보 입력란 숨기기' 버튼 숨김
		$('.hide-detail-info-panel-btn').css('display', 'none')
		// '상세 정보 입력란 보이기' 버튼 표기
		$('.show-detail-info-panel-btn').css('display', 'inline-block')
	}
}

const setCurrentLevelToMax = function() {
	var formdata = getFormData()
	var maxLevel = getMaxLevel(formdata.from.elite, formdata.rarity)
	
	$('#from_level').val(maxLevel)
}
