...
const makeOpForm = function (opID, options = {}) {
    ...
    html += `<div class="remove-op thumb rarity_${rarity}"><button class="no-margin square caution button remove-btn">제거</button><img alt=${db.op[opID].releaseOrder} src="./images/op/thumb/${db.op[opID].releaseOrder}.png?v=2"><div class="no-margin square describe-op"><span>${db.op[opID].name.kr}</span></div>`
    /*if (isModuleExists) {
...
const loadLocalStorage = function () {
    ...
    // 단, 프로모션의 경우 원본과 서로 붙어있을 수 있게 해야 함.
    // 따라서, 프로모션의 우선순위가 원본과 기본적으로 동일하되, 원본 바로 뒤여야 함.
    // 이를 구현하려면 프로모션의 정보를 원본 기준으로 조회하고, 마지막에 출시순서로 정렬하면 됨.
    existingProps.IDs.sort(function (ID1, ID2) {
        let ro1 = Number(ID1.split('-')[1] || 0), // 각각 ID1, ID2의 출시순서. 원본이라면 0으로 설정됨.
            ro2 = Number(ID2.split('-')[1] || 0)
        
        let a, b
        
        // 프로모션도 원본 정보 기준으로 정렬되게 하여 기본 우선순위를 원본과 동일하게 맞춤
        ID1 = ID1.split('-')[0]
        ID2 = ID2.split('-')[0]
    ...
    */
    ...
    if (checkModule) {
        ...
        for (let id in similarOps) {
            ...
            // 현재 오퍼레이터가 모듈을 갖고 있지 않다면 검증하지 않음
            // 일반적으로는, 모듈 변경 신호가 들어온 경우 당연히 모듈을 갖고 있겠지만
            // 아미야의 어레인지 버전일 경우 원본 아미야의 모듈 변경 신호를 같이 받았을 수 있음.
            if (!db.op[id].module) continue
            ...
            console.log(id,'모듈',module,'from',from, 'moduleCodes',moduleCodes)
    ...
    if (checkModule) {
        for (let id in similarOps) {
            ...
            for (let moduleCode in moduleCodes) {
                ...
                // id가 프로모션도 가리킬 수 있는지 나중에 확인 필요. 프로모션에 모듈 들어갔을 때 계산하려면 그래야 함.
                let currentModuleElm = document.querySelector(`.op[name="op_${id}"] .current-module-level[data-module-code=${moduleCode}]`),
                targetModuleElm = document.querySelector(`.op[name="op_${id}"] .target-module-level[data-module-code=${moduleCode}]`),
...
const addOpEventListener = function (opForm, doTrigger = false) {
    ...
    */
	...
	// opForm의 오퍼레이터 이름 폰트 크기 조절
	let opnameElm = opForm.querySelector('.describe-op')
	//$(opnameElm).textfill()
	textfill(opnameElm)
...
const showSearchResult = function () {
	...
	for (var i = 0; i < opnameArray.length; ++i) {
		form	+=	'<div class="search-result-op" name="' + 'op_' + opnameArray[i][1] + '">'
				+	'<img src="./images/op/thumb/' + db.op[opnameArray[i][1]].releaseOrder + '.png?v=2">'
...
const resizeObserver_textfill = new ResizeObserver((entries) => {
	entries.forEach(entry => {
		...
		if (entry.contentBoxSize) {
			if (entry.contentBoxSize[0]) {
				...
				console.log(`cbs[0], ${height} x ${width}`)
			}
			else {
				height = entry.contentBoxSize.blockSize
				width = entry.contentBoxSize.inlineSize
				console.log(`cbs, ${height} x ${width}`)
			}
			
		} else {
			height = entry.contentRect.height
			width = entry.contentRect.width
			console.log(`cr, ${height} x ${width}`)
		}*/
		textfill(entry.target)
	})
});


// elem: 크기를 조정할 span 요소의 부모 요소
function textfill(elem) {
	//return
	var ourText = elem.querySelector('span');
	function resizeText(){
		let fontSize, sizeByHeight, sizeByWidth;
		
		const pseudoVerticalPadding = 4, pseudoHorizontalPadding = 4; // 컨테이너에 너무 꽉 차지 않도록, 글자 크기 계산할 때 가상의 패딩 적용

		fontSize = parseFloat(window.getComputedStyle(ourText).getPropertyValue('font-size'));

		sizeByWidth = (fontSize * (elem.clientWidth - pseudoHorizontalPadding) / ourText.clientWidth); // (multiplier-0.1)

		sizeByHeight = (fontSize * (elem.clientHeight - pseudoVerticalPadding) / ourText.clientHeight); // (multiplier-0.1)
		
		ourText.style.fontSize = `${(sizeByWidth > 0 && sizeByHeight > sizeByWidth ? sizeByWidth : sizeByHeight)}px`;
	}
	resizeObserver_textfill.observe(elem)
	resizeText()
	resizeText()
}
/*
(function($) {
    $.fn.textfill = function(maxFontSize) {
        ...
        return this.each(function(){
			...
			//resizeObserver_textfill.observe(this)
