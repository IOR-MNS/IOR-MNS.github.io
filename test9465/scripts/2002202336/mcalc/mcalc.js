var itemOrder = {
	"keys": ["특수재료", "일반재료", "스킬개론", "칩류"/*, "건설자재"*/],
	
	"특수재료": [
		["D32강", "바이폴라 나노플레이크 칩", "중합제"],
		["RMA70-24",	"망간 중합체",	"화이트 호스 콜",	"고급 연마석",	"중합젤",	"열합금 덩어리"],
		["RMA70-12",	"망간 광석",		"로식 콜",			"연마석",		"젤라틴",	"열합금"]
	],
...
const showItem = function (item) {
	...
	for (var i = 0; i < itemOrder.keys.length; ++i) {
		...
		for (var j = 0; j < items.length; ++j) {
			for (var k = 0; k < items[j].length; ++k) {
				...
				var grade = itemData[itemID].grade
				...
				html += '<div align="center" class="item ' + 'grade_' + grade + '" id="item_' + items[j][k] + '"><div class="item_thumb"><img src="./images/item/inv_2/'
				...
				html += '</span><div class="item_thumb_quantity">0</div></div><div class="item_body"><div style="height:12px"></div><div class="item_body_have">보유량<br><input type="text" class="item_body_have_input" value="'
	...
	*/
	...
	$('.item').off('refresh').on('refresh', function () {
		...
		var craft = (itemData[ID].craft.length <= 0) ? [] : itemData[ID].craft[0]
