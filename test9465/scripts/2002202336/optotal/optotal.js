...
const addResultToMcalc = function () {
	var inputData = fetchInputData()
	if (inputData === false) {
		toastr.error('전송할 데이터가 존재하지 않음', '재료 계산기로 결과 보내기')
		return
	}
	
	if (!confirm('주의: 이 작업은 되돌릴 수 없습니다.\n결과 데이터를 전송하시려면 확인 버튼을 눌러주십시오.')) {
		toastr.error('취소됨', '재료 계산기로 결과 보내기')
		return
	}
	...
	toastr.success('성공적으로 결과 데이터를 전송함', '재료 계산기로 결과 보내기')
